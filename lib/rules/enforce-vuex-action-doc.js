...
// TODO: docs
const hasLeadingComment = (node, context) => {
  const comments = context.getCommentsBefore(node) || []
  return comments.some(({ value = "" }) => value.trim())
}

// TODO: docs
const findIdentifierValue = (shorthandNode, context) => {
  ...
  const actions =
    ...
    node.value.type === "ObjectExpression"
      ...
      : node.value.type === "Identifier"
        ? findIdentifierValue(node, context).properties
        : node.value.properties

  const invalidActions = actions.filter(
    action => !hasLeadingComment(action, context),
  )
