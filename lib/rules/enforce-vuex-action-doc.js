...
// TODO: docs
const findIdentifierProperties = (shorthandNode, context) => {
  const { variables = [] } = context.getScope(shorthandNode.value)

  const variable = variables.find(
    ({ name }) => shorthandNode.value.name === name,
  )
...
function checkActionsDoc(node, context) {
  ...
  const actions = R.cond([
    [R.equals("ObjectExpression"), R.always(node.value.properties)],
    [R.equals("Identifier"), R.always(findIdentifierProperties(node, context))],
    [R.T, R.always([])],
  ])(node.value.type)
